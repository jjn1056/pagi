...
Write your handlers this way unless you have a specific reason not to. The rest of
this section is the exception, not the rule.

=head2 Addendum: using IO::Async directly, if you accept the coupling

If you are running on L<PAGI::Server> and you do not mind tying your handler to
its loop, you can skip Future::IO and use L<IO::Async> directly -- its timers, and
the whole C<Net::Async::*> family of client modules. The reference server I<is>
IO::Async, so an IO::Async loop is always running, and a handler reaches it
through the ordinary constructor: C<< IO::Async::Loop->new >> returns the
already-running loop (a cached singleton), not a fresh one. So you add a client to
the loop that is already turning and C<await> it:
