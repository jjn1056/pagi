...
One limit to keep in mind: this hub lives in C<< $scope->{state} >>, which is
in-memory and B<per worker>. In multi-worker mode (C<pagi-server --workers N>)
each worker runs its own lifespan, and so its own ticker and hub -- there is no
tick shared across workers, and the C<count> differs between them. The example is
written for single-worker mode.

For anything beyond a worked example -- or to share one source across workers and
hosts -- reach for a real pub/sub layer backed by an external broker, such as the
C<PAGI::Middleware::Channels> distribution, rather than hand-rolling an in-memory
hub. The pattern above is what such a layer implements underneath.
