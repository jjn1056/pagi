...
the loop -- B<you> select the backend. For an IO::Async loop that is

    use Future::IO::Impl::IOAsync;

before loading any Future::IO-based libraries, so Future::IO resolves its
primitives on the loop you are running. (A server built on a different reactor
would have you load that reactor's Future::IO backend instead.)
