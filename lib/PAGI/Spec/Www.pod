...
C<0.2>: Added C<pagi.transport> outbound flow-control introspection
...
disconnect handling into one abnormal-only reason vocabulary, with
C<on_disconnect> (abnormal drops) and C<on_complete> (successful completion) on
the C<pagi.connection> object. Added keepalive events (C<websocket.keepalive>,
C<sse.keepalive>), SSE support for all HTTP methods (not just GET) with the
C<sse.request> receive event, and the C<pagi.connection> scope key for
non-destructive disconnect detection. Documented the server-supplied HTTP, SSE,
and WebSocket behaviors and WebSocket over HTTP/2 (RFC 8441). B<Removed>
...
If the client disconnects or cancels the connection, servers MUST send an explicit C<disconnect> event to the application and update the connection state (see L</Connection State>).
...
Exactly one of C<on_disconnect> and C<on_complete> fires for a given request.
...
C<code> (Int) -- WebSocket close code per RFC 6455. When the peer sent a close frame, this is its code (or C<1005>, "no status received", if the frame carried none). When the connection dropped or was aborted with no close handshake -- timeout, write error, server shutdown -- servers MUST report C<1006> ("abnormal closure").

=item -

C<reason> (String, default empty) -- Disconnect reason. For an abnormal close, servers MUST set this to the matching standard reason string from L</Standard Disconnect Reasons>. For a clean client-initiated close (code 1000) it MAY be empty.

=back

The C<code> carries the RFC 6455 close code; C<reason> supplements it with a standard reason token so an application can branch on a single vocabulary across HTTP, WebSocket, and SSE:

    Code  Reason             Meaning
    ----  -----------------  -------
    1000  (empty)            Normal closure initiated by client
    1001  server_shutdown    Server going away (shutdown/restart)
    1001  client_closed      Client navigating away
    1006  keepalive_timeout  No pong received within timeout
    1006  write_timeout      Send operation timed out
    1006  write_error        Write to socket failed
    1007  protocol_error     Text frame contained invalid UTF-8
...
server raises, failing the C<send>. C<retry> MUST be a non-negative integer.
...
C<0.2> (Draft): C<pagi.transport> flow control; C<websocket.http.response>
denial-response extension; C<on_complete> plus reworked abnormal-only disconnect
reasons; keepalive events; SSE POST method support (C<sse.request>);
C<pagi.connection>; documented server-supplied behaviors and WebSocket over
HTTP/2; clarified scope fields. Removed C<pagi.features> and the per-send
C<timeout> field.
