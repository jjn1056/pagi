...
C<0.2>: Added keepalive events (C<websocket.keepalive>, C<sse.keepalive>), disconnect reasons, SSE support for all HTTP methods (not just GET) with C<sse.request> receive event for request body. Added C<pagi.connection> scope key for non-destructive disconnect detection.
...
when the application did not: C<Content-Type: text/event-stream> (if absent) and
C<Cache-Control: no-cache>. Over HTTP/1.1 it additionally sends
C<Connection: keep-alive> and a C<Date> header and frames the stream with chunked
C<Transfer-Encoding>; over HTTP/2 those transport headers do not apply (the stream
is multiplexed and DATA-framed). Applications need not set any of these.
...
C<0.2> (Draft): SSE POST method support, keepalive events, disconnect reasons,
