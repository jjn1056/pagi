...
Use package-level variables (C<my $db;> outside the sub) to share resources between lifespan and request handlers within the same process. You cannot pass data via C<$scope> because each request gets a separate scope hash.
